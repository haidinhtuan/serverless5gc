...
\begin{abstract}
Open-source 5G core implementations deploy network functions as always-on processes that consume resources even when idle---a significant inefficiency for private and edge deployments with sporadic traffic. This paper presents \textit{Serverless5GC}, an architecture that maps each 3GPP control-plane procedure to an independent Function-as-a-Service invocation, enabling scale-to-zero operation without modifying the standard N2 interface. The system decomposes 12~network functions (Release~15--17) into 31~serverless procedures, fronted by an SCTP/NGAP proxy that bridges unmodified RAN equipment to an HTTP-based serverless backend. Evaluation against Open5GS and free5GC across five traffic scenarios (idle to 20~registrations/s burst) shows that Serverless5GC achieves median registration latency of 406--522\,ms---on par with the C-based Open5GS baseline (403--606\,ms)---while maintaining 100\% success across 3{,}000~registrations. The architecture consolidates all core logic onto a single VM at 0.002~GB-seconds per registration, with memory consumption virtually independent of load ($+$1.6\% from idle to burst). Even under worst-case cold-start conditions where all 31~function pods are evicted simultaneously, the system sustains zero failures and converges to warm-start latency within 4--5~seconds. These results demonstrate that procedure-level serverless decomposition of the 5G core is practically viable, offering resource efficiency unattainable with always-on architectures while preserving signaling reliability.
\end{abstract}

\begin{IEEEkeywords}
5G core network, serverless computing, Function-as-a-Service, procedure-based decomposition, scale-to-zero
\end{IEEEkeywords}
